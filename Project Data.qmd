...
vars_acs5 <- load_variables(year = 2022, dataset = "acs5")

vars_acs5_profile <- load_variables(year = 2022, dataset = "acs5/profile")

vars_acs5_subject <- load_variables(year = 2022, dataset = "acs5/subject")
...
census_bureau_data_clean <- mutate(
  ...
  race_hispanicE =
    round((race_hispanicE/total_populationE)*100, 2),
...
## Department of Transportation Data Import

```{r DOT Data}
library(stringr)
dot <- read.csv("UseThis_Tracts_Stops_Xsctns.csv")
dot$GEOID <- str_split_i(dot$GEOIDFQ, "US",2)
  #Compiled # of road intersections from DOT'S Arnold dataset in ArcGIS, and # of transit stops from DOT'S National Transit Map dataset. Standardized to tract level by calculating rate per acre. 

  # For Ben - fix leading zeroes problem

dot_clean <- select(dot, c("GEOID","NAMELSAD","Number.of.Intersections", "Number.of.Transit.Stops", "Acres","Intersection.Per.Acre","Stops.Per.Acre"))

#Add column for square miles per census tract
dot_clean <- mutate(
  dot_clean,
  sq_mi = Acres * 0.0015625
)
...
model_data <- left_join(census_bureau_data_clean, dot_clean)

#Add column for population density (people per sq mile)
model_data <- mutate(
  model_data,
  pop_density = round(total_populationE/sq_mi,0)
)
...
write.csv(model_data, file = "Transportation_Data_merged.csv")
